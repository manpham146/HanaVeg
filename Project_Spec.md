...
- **Backend/Cơ sở dữ liệu:** Supabase (PostgreSQL + Auth + Storage). Auth qua `@supabase/ssr`.
- **Đa ngôn ngữ (i18n):** `next-intl` — hỗ trợ 3 ngôn ngữ: Tiếng Việt (mặc định), English, 中文.
- **Font:** `@fontsource/playfair-display` (restaurant headings), `@fontsource/inter` (admin), `@fontsource/jost` (restaurant body).
...
- **Bảng màu Restaurant (Thực tế — CSS Variables trong `:root`):**
  - Primary: Xanh lá đậm (`#0F1F15`) — text chính, nút bấm
  - Secondary: Gold/Beige (`#D4B274`) — điểm nhấn, nút CTA
  - Background: Cream ấm (`#F6EFDF`) — nền trang
  - Foreground: Đen lá (`#0B1C10`) — text cơ bản
  - Muted: Beige nhạt (`#E8DDCA`) — nền phụ, hover
  - Border: Beige trung (`#DBCDB3`) — viền, divider
  - Destructive: Đỏ (`#DD4C4F`) — cảnh báo, xóa
  - Surface: Kem nhạt (`#FAF6EE`) — card nổi
  - Dark mode: đảo ngược — nền `#0B1C10`, primary `#D4B274`, card `#15261C`
...
### B. Trang Quản trị (Admin Panel - Theme riêng biệt)

Admin Panel sử dụng **bộ theme riêng hoàn toàn**, tách biệt với giao diện nhà hàng:

- **Font chữ:** Inter (400/500/600/700) — thay vì Playfair Display của trang nhà hàng. Wrap trong `.font-inter` class.
- **Layout:** `SidebarProvider` + `SidebarInset` + `AdminBreadcrumb`. Route group `(admin)` có layout riêng.
- **Bảng màu Admin (CSS Variables override qua `.font-inter`):**
  - Background: `#FFFFFF` (trắng tinh — khác với Cream `#F6EFDF` của restaurant)
  - Foreground: `#0f172a` (slate-900)
  - Primary: `#D4A100` (Gold) — dùng cho nút bấm, ring, active states
  - Muted: `#f5f5f4` — nền nhẹ cho badges, hover
  - Border/Input: `#e5e5e5` — viền nhẹ nhàng
  - Destructive: `#DD4C4F` (đỏ) — dùng cho nút xóa, cảnh báo
  - Border-radius: `0.5rem`
- **Sidebar (Dark theme):**
  - Background: `#0B1C10` (xanh lá đậm rất tối)
  - Foreground: `#e8e8e0` (sáng)
  - Primary: `#D4A100` (Gold highlight)
  - Accent: `#142a1a` (hover state)
  - Border: `#1e3a24`
- **UI Components (shadcn/ui):** Sử dụng component props chuẩn, KHÔNG custom inline style:
  - `AlertDialog`: dùng `size="sm"`, `AlertDialogMedia`, `variant="destructive"/"outline"`
  - `Sheet`: cho form tạo/sửa, mở từ bên phải
  - `Select`, `Badge`, `Checkbox`, `Table` theo chuẩn shadcn
- **Auth Guard:** Server-side check trong `admin/layout.tsx` — kiểm tra `supabase.auth.getUser()` + role `admin` từ bảng `profiles`. Redirect về `/login` nếu không hợp lệ.
...
### C. Lược đồ CSDL (Supabase PostgreSQL — Đã triển khai)

- **Bảng `menu_categories`:** `id` (uuid), `name` (chuỗi, tiếng Việt mặc định), `name_en`, `name_zh`, `sort_order` (số), `created_at`.
- **Bảng `menu_items`:** `id` (uuid), `name`, `name_en`, `name_zh`, `description`, `description_en`, `description_zh`, `price` (số), `image_url` (chuỗi, Supabase Storage), `category_id` (FK → `menu_categories`), `is_available` (boolean), `sort_order`, `created_at`.
- **Bảng `blog_categories`:** `id` (uuid), `name`, `name_en`, `name_zh`, `slug`, `created_at`.
- **Bảng `blog_posts`:** `id` (uuid), `title`/`title_en`/`title_zh`, `subtitle`/`subtitle_en`/`subtitle_zh`, `slug`, `content`/`content_en`/`content_zh`, `cover_image`, `category_id` (FK), `author_id` (FK), `is_published`, `published_at`, `created_at`.
- **Bảng `profiles`:** `id` (uuid, FK → `auth.users`), `email`, `role` (enum: `admin` | `staff`), `full_name`.
- **Bảng `bookings`:** `id` (uuid), `customer_name`, `phone`, `booking_date`, `booking_time`, `guest_count`, `status` (enum), `notes`.
- **Storage bucket:** `menu-images` — lưu hình món ăn, public access.
